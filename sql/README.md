...
Raw-data setup: [`data/raw/README.md`](data/raw/README.md)
...
Calibration bins, favourite-longshot ROI, validation comparisons, and bootstrap confidence intervals are intentionally calculated in Python rather than duplicated as SQL marts.
...
Local PostgreSQL credentials are stored in `.env`, which is excluded from version control. Both SQL-backed notebooks use the shared `src/db.py` helper so the connection setup is defined in one place.
